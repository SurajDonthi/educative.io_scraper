...
```
Note: 1. Updates Information
      -  AutoFixUrl and AutoResume were added (v3.6.5+)
      i. AutoFixUrl: The urls text file will be automatically updated based on the last url from Log File. If a manual edit was done on the text file then consider unchecking this option.
      ii. AutoResume (Recommended): Retries 3/4 times if there is any error occurred for a specific url. If it fails still, check the log for more details. 
